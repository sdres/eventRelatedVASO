...
for sub in SUBS:
    ...
    for ses in ['ses-001']:
        ...
        if not sub == 'sub-07' and not (sub == 'sub-09' and ses == 'ses-01'):
            # Make overall t1w
        ...
        if sub == 'sub-07':
            for runType in ['blockStim', 'eventStim']:

                dataFiles = sorted(glob.glob(f'{outFolder}/*{runType}*_moco.nii'))

                header = nb.load(dataFiles[0]).header
                affine = nb.load(dataFiles[0]).affine
...
outFolder = f'{ROOT}/derivativesTestTest/{sub}/{ses}/func'
        ...
        if sub == 'sub-09' and ses == 'ses-001':
            # Make overall t1w
            dataFiles = sorted(glob.glob(f'{outFolder}/*LongTR*_moco.nii'))
