...
for sub in ['sub-05']:
    runs = sorted(glob.glob(f'{root}/{sub}/ses-00*/func/{sub}_ses-00*_task-event*_run-002_cbv.nii.gz'))
    # for focus in ['v1', 's1']:
    for focus in ['v1']:
        for run in runs:
            ...
            for modality in ['BOLD', 'VASO']:
                ...
                for timepoint in range(1,11):
                    try:
                        dataFile = f'{root}/derivatives/{sub}/{ses}/old/{base}_{focus}_{modality}_layers_FIR.feat/stats/pe{timepoint}.nii.gz'
