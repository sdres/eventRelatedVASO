...
for focus in ['v1']:
    ...
    for sub in subs:
        outFolder = f'{root}/derivativesTest/{sub}/{ses}/upsampledFunctional'
        if sub == 'sub-07':
            ...
            for run in runs:
                ...
                for modality in modalities:
                    ...
                    for j in idx_layers:  # Compute bin averages
                        ...
                        df.to_csv(f'{root}/derivativesTest/{sub}/{ses}/{base}_{focus}_{modality}_layer{j:02d}.csv', index=False)

                        for i, tp in enumerate(mask_mean):
                            layerTS[j-1,0,0,i] = tp

                    nii = nb.Nifti1Image(layerTS, header = header, affine = affine)
                    nb.save(nii, f'{root}/derivativesTest/{sub}/{ses}/{base}_{focus}_{modality}_layers_ts.nii')


                    mask_mean = np.mean(data[idxMask],axis=0)[0]
                    df = pd.DataFrame({'signal':mask_mean})
                    df.to_csv(f'{root}/derivativesTest/{sub}/{ses}/{base}_{focus}_{modality}.csv', index=False)

                    mask_mean = mask_mean.reshape((1,1,1,mask_mean.shape[-1]))
                    nii = nb.Nifti1Image(mask_mean, header = header, affine = affine)
                    nb.save(nii, f'{root}/derivativesTest/{sub}/{ses}/{base}_{focus}_{modality}_ts.nii')
